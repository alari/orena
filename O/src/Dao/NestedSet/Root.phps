...
abstract class O_Dao_NestedSet_Root extends O_Dao_ActiveRecord {
	...
	public function injectTop( O_Dao_NestedSet_Node $node )
	{
		...
		$this->nodes->query()->field( "left_key", "left_key+" . $_2N, true )->field( "right_key", "right_key+" . $_2N, 
				true )->update();
	...
	public function injectBottom( O_Dao_NestedSet_Node $node )
	{
		...
		$nodeQuery->field( "root", $this->id )->field( "left_key", "left_key+" . $maxRight, true )->field( "right_key", 
				"right_key+" . $maxRight, true )->update();
		$node->reload();
	}
