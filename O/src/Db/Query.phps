...
class O_Db_Query {
	...
	public function tableExists()
	{
		if (!isset( $this->from[ 0 ] )) {
			throw new O_Ex_WrongArgument( "No table specified for query." );
		}
		$table_name = $this->from[ 0 ];
		if (strpos( $table_name, " " ))
			$table_name = substr( $table_name, 0, strpos( $table_name, " " ) );
		$stmt = O_Db_Manager::getConnection()->query(
	...
	{
		$this->where[] = Array ("t" => self::T_WHERE_TEST, "param" => $value, "cond" => $type,
								"field" => $field);
		return $this;
	}
	 ...
	 *
	 * @param bool $returnLastId true: return last auto_increment id, false: return count of affected rows
	...
	protected function bindParams( $query )
	{
		foreach ($this->params as $k => $v) {
			...
			elseif (is_double( $v ) || is_float( $v ) || is_numeric(
