 ...
 * cmd/show/registry -- registry key to get resource (if source is set to "field")
 ...
 * cmd/show/url_pattern -- pattern to build url via command, with {PAGE} and {ORDER} placements, and other {...}'s for resource field displaying
 * cmd/show/url_callback -- static callback to build url with
 * (if no url callback is provided, resource->url will be used)
...
class O_Cmd_ShowLoop extends O_Command {
	...
	public function process()
	{
		...
		switch ($type) {
			...
			case "field" :
				if (!$resource) {
					$resource = O_Registry::get( O_Registry::get( "app/cmd/show/registry" ) );
				}
		...
		if ($this->url_pattern) {
			...
			$url_callback = array ($this->resource, "url");
		}
	...
	public function replaceInUrlPattern( $match )
	{
		...
		if ($m == "PAGE" || $m == "ORDER")
			return $match[ 0 ];
		return urlencode( $this->resource->$m );
	}
